...
// SMTP is configured entirely through the container's environment (see Dockerfile / README) and read
// lazily at send time, so a missing config surfaces as a send error rather than a crash at boot (and
// tests can set the env before the first send). The transport is cached after the first use.
class MailService {
	static #transporter = null;

	static #readSmtpConfig() {
	...
	static isConfigured() {
		const { host, from } = this.#readSmtpConfig();
		return Boolean(host && from);
	}

	static #getTransport() {
		const { host, port, user, pass, secure } = this.#readSmtpConfig();
		if (!host) {
